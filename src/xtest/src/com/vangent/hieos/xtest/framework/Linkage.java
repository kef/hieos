...
public class Linkage {
    ...
    public OMElement get_log_file(String test_dir)
            throws XdsInternalException {
        if (debug) {
            System.out.println("LogFileName is " + getLogFileName(test_dir));
        }
        OMElement rootNode = XMLParser.fileToOM(getLogFileName(test_dir));
        return rootNode;
    }
    ...
    HashMap<String, String> compileUseXPath() throws XdsException {
        ...
        for (int i = 0; i < use_xpath.size(); i++) {
            ...
            try {
                if (is_testdir) {
                    OMElement root = XMLParser.fileToOM(getLogFileName(testdir));
                    ...
                    String result = XPathHelper.stringValueOf(step_ele, xpath, null);
                    ...
                    OMElement root = XMLParser.fileToOM(file);
                    String result = XPathHelper.stringValueOf(root, xpath, null);
                    linkage.put(symbol, result);
                    if (metadata_ele != null) {
                        replace_string_in_text_and_attributes(metadata_ele, symbol, result);
                    }
    ...
    public HashMap<String, String> compileUseId() throws XdsException,
        ...
        for (int i = 0; i < use_id.size(); i++) {
            ...
            if (by_value) {
                linkage.put(symbol, value);
                if (metadata_ele == null) {
                    throw new XdsException("metadata_ele is null");
                }
                if (metadata_ele != null) {
