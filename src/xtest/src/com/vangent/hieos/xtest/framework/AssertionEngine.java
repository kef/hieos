    ...
    void buildDataModel() throws XMLParserException, XdsInternalException {
        ...
        for (DataRef ref : refs) {
            ...
            if (ref.file.equals("THIS")) {
                ...
                try {
                    file_root = XMLParser.fileToOM(ref.file);
                } catch (Exception e) {
                    // hmmm, input may be in log directory, try that
                    file_root = XMLParser.fileToOM(TestConfig.log_dir + "/" + ref.file);
                }
            }
    ...
    public void run(ErrorReportingInterface err, OMElement assertion_output) {
        ...
        try {
            for (Assertion assertion : assertions) {
                String result = XPathHelper.stringValueOf(data, assertion.xpath, null);
                if (result == null || !result.toLowerCase().equals("true")) {
                    ...
                    if (equals_index > 0) {
                        ...
                        String left_side_value = XPathHelper.stringValueOf(data, left_side_xpath, null);
                        String right_side_value = XPathHelper.stringValueOf(data, right_side_xpath, null);
