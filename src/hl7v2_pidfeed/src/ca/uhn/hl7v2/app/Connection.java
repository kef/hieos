...
public class Connection {
    ...
    public Connection(Parser parser, LowerLayerProtocol llp, Socket bidirectional) throws LLPException, IOException {
        ...
        //init(parser);
        init(parser, bidirectional);
        // +++++ HEIOS CHANGES END HERE +++++
    ...
    // +++++ HIEOS CHANGES BEGIN HERE +++++
    // Replaces init method above in order to place socket in responder
    private void init(Parser parser, Socket bidirectional) throws LLPException {
        this.parser = parser;
        sockets = new ArrayList();
        receipts = new HashMap();
        receivers = new ArrayList();
        responder = new Responder(parser, bidirectional);
    }
