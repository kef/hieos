...
public class Responder {
    ...
    // +++++ HIEOS CHANGES BEGIN HERE +++++
    public Responder(Parser parser, Socket socket) throws LLPException
    {
        this(parser);
        this.socket = socket;
    }
    ...
    protected String processMessage(String incomingMessageString) throws HL7Exception {
        ...
        if (outgoingMessageString == null) {
            try {
                ...
                {
                    response = ((MessageTypeRouter)app).processMessage(incomingMessageObject, this.socket);
                } else {
                    response = app.processMessage(incomingMessageObject);
                }
