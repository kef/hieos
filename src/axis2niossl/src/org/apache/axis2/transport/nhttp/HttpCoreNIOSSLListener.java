 ...
 * under the License.
 */
// MODIFIED BY HIEOS (marked below).
...
public class HttpCoreNIOSSLListener extends HttpCoreNIOListener {
    ...
    protected SSLContext getSSLContext(TransportInDescription transportIn) throws AxisFault {
        ...
        if (keyParam != null) {
            ...
            try {
                ...
                // HIEOS/AMS changed default behavior to retrieve certificate store access information from environment variables
                // If unsuccessful, fallback to whatever has been provided in axis2.xml
                String keyStoreLocation = System.getProperty("javax.net.ssl.keyStore");
                URL keyStoreURL = getCertificateStoreURL(keyStoreLocation);
                if (keyStoreURL != null) {
                    // if javax.net.ssl.keyStore has been set, retrieve passwords from related env. variables.
                    // HIEOS/AMS, At some point get store type from env i.e JKS
                    storePassword = System.getProperty("javax.net.ssl.keyStorePassword");
                    keyPassword = System.getProperty("javax.net.ssl.keyStorePassword");     
                } else {
                  // Fallback to defaults as configured in axis2.xml
                    keyStoreURL = getCertificateStoreURL(location);
                }
                log.debug("Loading Key Store from URL : " + keyStoreURL);

                keyStore.load(keyStoreURL.openStream(), storePassword.toCharArray());
        ...
        if (trustParam != null) {
            ...
            try { 
                ...
                // HIEOS/AMS changed default behavior to retrieve certificate store access information from environment variables
                // Fallback to whatever has been provided in axis2.xml
                String trustStoreLocation = System.getProperty("javax.net.ssl.trustStore");
                URL trustStoreURL = getCertificateStoreURL(trustStoreLocation);
                if (trustStoreURL != null) {
                    // if javax.net.ssl.trustStore has been set, retrieve passwords from related env. variables.
                    // HIEOS/AMS, At some point get store type from env i.e JKS ??
                    storePassword = System.getProperty("javax.net.ssl.trustStorePassword");
                } else {
                  // Fallback on defaults as configured in axis2.xml
                    trustStoreURL = getCertificateStoreURL(location);
                }
                log.debug("Loading Trust Key Store from URL : " + trustStoreURL);

                trustStore.load(trustStoreURL.openStream(), storePassword.toCharArray());
                ...
                log.error("Error loading Trust store : " + location, gse);
                throw new AxisFault("Error loading Trust store : " + location, gse);
            } catch (IOException ioe) {
                log.error("Error opening Trust store : " + location, ioe);
                throw new AxisFault("Error opening Trust store : " + location, ioe);
            }
        }
    ...
    // Method created by HIEOS/AMS
    /**
     * Create an URL object given a file system path for a certificate store.
     *
     * @param certificateStoreFileSystemPath a string representing the file system path.
     * @return URL
     */
    private URL getCertificateStoreURL(String certificateStoreFileSystemPath) {
        URL certificateStoreLocationURL = null;
        if (certificateStoreFileSystemPath != null && certificateStoreFileSystemPath.trim().length() > 0) {
            // URL url = getClass().getClassLoader().getResource(location);
            File f = new File(certificateStoreFileSystemPath);
            if (f.isFile()) {
                try {
                    certificateStoreLocationURL = f.toURL();
                } catch (MalformedURLException ex) {
                    // Eating exception.
                    log.warn(certificateStoreFileSystemPath + " is not a valid path to a certificate store.", ex);
                }
            }
        }
        return certificateStoreLocationURL;
    }
}
