...
public abstract class QueryContents {
    ...
    protected void displayStructureHeader(int index, XView xv) {
        ...
        if (request_xml != null) {
            buf.append(" (").append(xv.build_query_request_xml_link(Integer.toString(index))).append(")");
        }
        if (result_xml != null) {
            buf.append(" (").append(xv.build_query_results_xml_link(Integer.toString(index))).append(")");
        }
        if (hasExceptions()) {
            buf.append(xv.h().link("___except", "/xwebtools/xviewer/exceptions?cntl=" + index));
        }
        if (hasErrors()) {
            buf.append(xv.h().link("___error", "/xwebtools/xviewer/errors?cntl=" + index));
        }
        if (hasFatalErrors()) {
            buf.append(xv.h().link("___fatal", "/xwebtools/xviewer/fatal?cntl=" + index));
        }
